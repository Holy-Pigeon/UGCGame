            ...
            if (ClientSocket.IsValid())
            {
                ...
                while (bRunning)
                {
                    ...
                    if (ClientSocket->Recv(Buffer, sizeof(Buffer) - 1, BytesRead))
                    {
                        ...
                        {
                            // Accept both the legacy "type" field used by the Python server
                            // and the newer "command" field used by some direct socket clients.
                            FString CommandType;
                            const bool bHasCommandType =
                            ...
                            if (bHasCommandType)
                            {
                                ...
                                // Match the newline-delimited framing used by the alternate codepath.
                                Response += TEXT("\n");
                            ...
                            {
                                UE_LOG(LogTemp, Warning, TEXT("MCPServerRunnable: Missing 'type' or 'command' field in command"));
                            }
                        }
                        else
                        {
